...
begin "constans"
    ...
    const T_w = 13.86 + 273.15 # Evaporator wall temperature
...
begin "kernel"
    ...
    Δy₀_ = 0.0001
    const IR = 1.1
    n_grid_x = ceil(log(1.0 - (0.5 / Δx₀_) * (1.0 - IR)) / log(IR))
    const Δx₀ = 0.5 * (1.0 - IR) / (1.0 - IR ^ n_grid_x)
    n_grid_y = ceil(log(1.0 - (0.5 / Δy₀_) * (1.0 - IR)) / log(IR))
    const Δy₀ = 0.5 * (1.0 - IR) / (1.0 - IR ^ n_grid_y)
    Δx = vcat(IR .^ collect(0:n_grid_x-1) .* Δx₀, reverse(IR .^ collect(0:n_grid_x -1 ) .* Δx₀))
    Δy = vcat(IR .^ collect(0:n_grid_y-1) .* Δy₀, reverse(IR .^ collect(0:n_grid_y - 1) .* Δy₀))
    const M = length(Δx)   # 20 nodes - Forward nodes 
    const N = length(Δy) - 1  # 19 nodes - Intermediate nodes
    const K1 = γ .* Δx ./ Δy.^2
    ...
    const Θꜛₛₒₗ_0 = vcat(0.0,cumsum(Δx)) .* (ΔT_supersub / ΔTₛₒₗ_ᵣ)
...
function _solve_ξ!(ξꜛₛₒₗ,ξꜛₛₒₗ_ᵢₙₜ,B,α_yₛₒₗ)
    ...
    for i in 1:M
        Du_ξₛₒₗ = [β_ξₛₒₗ[k] / α_yₛₒₗ[k] for k in 1:N] # Upper diagonal of ξₛₒₗ
        Dl_ξₛₒₗ = copy(Du_ξₛₒₗ) # Lower diagonal of ξₛₒₗ
        D_ξₛₒₗ = [-1.0 - 2β_ξₛₒₗ[p] / α_yₛₒₗ[p] for p in 1:N] # Diagonal of ξₛₒₗ
        B_ξₛₒₗ = copy(B) # Right hand side of ξₛₒₗ
        @inbounds @simd for j in 1:N
            B_ξₛₒₗ[j] = -view(ξꜛₛₒₗ,1:N+2,i)[j+1] - β_ξₛₒₗ[j] * ((1.0 - γ) / γ) * (view(ξꜛₛₒₗ,1:N+2,i)[j+2] - 2 * view(ξꜛₛₒₗ,1:N+2,i)[j+1] + view(ξꜛₛₒₗ,1:N+2,i)[j]) / α_yₛₒₗ[j]
        end
        @inbounds BC!(Dl_ξₛₒₗ,D_ξₛₒₗ,Du_ξₛₒₗ,B_ξₛₒₗ,ξꜛₛₒₗ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc()) # Neumann BC at y = 1.0
        @inbounds BC!(Dl_ξₛₒₗ,D_ξₛₒₗ,Du_ξₛₒₗ,B_ξₛₒₗ,∂ξꜛₛₒₗ_∂Yꜛₛₒₗ_0,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc()) # Neumann BC at y = 0.0
        @inbounds BC_Dirichlet!(view(ξꜛₛₒₗ,:,i+1),ξꜛₛₒₗ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc())
        TDMA!(Dl_ξₛₒₗ,D_ξₛₒₗ,Du_ξₛₒₗ,B_ξₛₒₗ,view(ξꜛₛₒₗ,2:N+1,i+1),N)
        @inbounds BC_Numan!(view(ξꜛₛₒₗ,:,i+1),∂ξꜛₛₒₗ_∂Yꜛₛₒₗ_0,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc())
    end
end
...
function _solve_Θ_sol!(Θꜛₛₒₗ,Θꜛₛₒₗ_ᵢₙₜ,α_yₛₒₗ)
    ...
    for i in 1:M
        Du_Θₛₒₗ = [β_Θₛₒₗ[k] / α_yₛₒₗ[k] for k in 1:N] # Upper diagonal of Θₛₒₗ
        Dl_Θₛₒₗ = copy(Du_Θₛₒₗ) # Lower diagonal of Θₛₒₗ
        D_Θₛₒₗ = [-1.0 - 2β_Θₛₒₗ[p] / α_yₛₒₗ[p] for p in 1:N] # Diagonal of Θₛₒₗ
        B_Θₛₒₗ = copy(B) # Right hand side of Θₛₒₗ
        @inbounds @simd for j in 1:N
            B_Θₛₒₗ[j] = -view(Θꜛₛₒₗ,1:N+2,i)[j+1] - β_Θₛₒₗ[j] * ((1.0 - γ) / γ) * (view(Θꜛₛₒₗ,1:N+2,i)[j+2] - 2 * view(Θꜛₛₒₗ,1:N+2,i)[j+1] + view(Θꜛₛₒₗ,1:N+2,i)[j]) / α_yₛₒₗ[j]
        end
        @inbounds BC!(Dl_Θₛₒₗ,D_Θₛₒₗ,Du_Θₛₒₗ,B_Θₛₒₗ,Θꜛₛₒₗ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        @inbounds BC!(Dl_Θₛₒₗ,D_Θₛₒₗ,Du_Θₛₒₗ,B_Θₛₒₗ,Θꜛₛₒₗ_0[i+1],Δy[1],Dirichlet(),StartLoc()) # Dirichlet BC at y = 0.0
        @inbounds BC_Dirichlet!(view(Θꜛₛₒₗ,:,i+1),Θꜛₛₒₗ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        @inbounds BC_Dirichlet!(view(Θꜛₛₒₗ,:,i+1),Θꜛₛₒₗ_0[i+1],Δy[1],Dirichlet(),StartLoc()) # Dirichlet BC at y = 0.0
...
function _solve_Θ_a!(Θꜛₐ,Θꜛₐ_ᵢₙₜ,α_yₐ)
    ...
    for i in 1:M
        Du_Θₐ = [β_Θₐ[k] / α_yₐ[k] for k in 1:N] # Upper diagonal of Θₐ
        Dl_Θₐ = copy(Du_Θₐ) # Lower diagonal of Θₐ
        D_Θₐ = [-1.0 - 2β_Θₐ[p] / α_yₐ[p] for p in 1:N] # Diagonal of Θₐ
        B_Θₐ = copy(B) # Right hand side of Θₐ
        @inbounds @simd for j in 1:N
            B_Θₐ[j] = -view(Θꜛₐ,1:N+2,i)[j+1] - β_Θₐ[j] * ((1.0 - γ) / γ) * (view(Θꜛₐ,1:N+2,i)[j+2] - 2 * view(Θꜛₐ,1:N+2,i)[j+1] + view(Θꜛₐ,1:N+2,i)[j]) / α_yₐ[j]
        end
        @inbounds BC!(Dl_Θₐ,D_Θₐ,Du_Θₐ,B_Θₐ,Θꜛₐ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        @inbounds BC!(Dl_Θₐ,D_Θₐ,Du_Θₐ,B_Θₐ,∂Θꜛₐ_∂Yꜛₐ,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc()) # Neumann BC at y = 0.0
        @inbounds BC_Dirichlet!(view(Θꜛₐ,:,i+1),Θꜛₐ_ᵢₙₜ[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        TDMA!(Dl_Θₐ,D_Θₐ,Du_Θₐ,B_Θₐ,view(Θꜛₐ,2:N+1,i+1),N)
        @inbounds BC_Numan!(view(Θꜛₐ,:,i+1),∂Θꜛₐ_∂Yꜛₐ,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc()) # Neumann BC at y = 0.0
    end
end
...
function _solve_ω!(ωꜛₐ,ωꜛ_int,α_yₐ)
    ...
    for i in 1:M
        Du_ωₐ = [β_ωₐ[k] / α_yₐ[k] for k in 1:N] # Upper diagonal of ωₐ
        Dl_ωₐ = copy(Du_ωₐ) # Lower diagonal of ωₐ
        D_ωₐ = [-1.0 - 2β_ωₐ[p] / α_yₐ[p] for p in 1:N] # Diagonal of ωₐ
        B_ωₐ = copy(B) # Right hand side of ωₐ
        @inbounds @simd for j in 1:N
            B_ωₐ[j] = -view(ωꜛₐ,1:N+2,i)[j+1] - β_ωₐ[j] * ((1.0 - γ) / γ) * (view(ωꜛₐ,1:N+2,i)[j+2] - 2 * view(ωꜛₐ,1:N+2,i)[j+1] + view(ωꜛₐ,1:N+2,i)[j]) / α_yₐ[j]
        end
        @inbounds BC!(Dl_ωₐ,D_ωₐ,Du_ωₐ,B_ωₐ,ωꜛ_int[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        @inbounds BC!(Dl_ωₐ,D_ωₐ,Du_ωₐ,B_ωₐ,∂ωꜛₐ_∂Yꜛₐ,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc()) # Neumann BC at y = 0.0
        @inbounds BC_Dirichlet!(view(ωꜛₐ,:,i+1),ωꜛ_int[i],Δy[end],N,Dirichlet(),EndLoc()) # Dirichlet BC at y = 1.0
        TDMA!(Dl_ωₐ,D_ωₐ,Du_ωₐ,B_ωₐ,view(ωꜛₐ,2:N+1,i+1),N)
        @inbounds BC_Numan!(view(ωꜛₐ,:,i+1),∂ωꜛₐ_∂Yꜛₐ,0.5 * (Δy[1] + Δy[2]),Neumann(),StartLoc()) # Neumann BC at y = 0.0
    end
end
...
function solve_domain!(X,ξꜛₛₒₗ,Θꜛₛₒₗ,Θꜛₐ,ωꜛₐ,ξ_int,T_int,Θꜛₐ_ᵢₙₜ,ωꜛ_int,B,α_yₛₒₗ,α_yₐ)
    ...
    # --------------------------------------------------------------------------------------------
    first_der_at_int = (x , x⁻ , x⁻⁻) -> (3x - 4x⁻ + x⁻⁻) / (2 * 0.5 * (Δy[1] + Δy[2]))
...
begin "test"
    ...
    B = zeros(Float64,N);
    cumsum_y = cumsum(Δy)
    α_yₐ = [Uꜛₐ(cumsum_y[i]) for i in 1:N];
    α_yₛₒₗ = [Uꜛₛₒₗ(cumsum_y[i]) for i in 1:N];
   
    cumsum_y_0 = vcat(0.0,cumsum_y)
    @. Θꜛₛₒₗ[:,1] = -1.5 * cumsum_y_0^2 + 3.0 * cumsum_y_0 ; # Θₛₒₗ at x = 0
    # Θꜛₛₒₗ[:,1] .= 1.0;
    Θꜛₐ[:,1] .= 1.0; # Θₐ at x = 0
...
plot(Θꜛₐ[:,end] .* (Tₐ_ᵢₙ .- T_w) .+ T_w .- 273.15, title = "Θꜛₐ_out")
plot(ωꜛₐ[:,end] .* ωₐ_ᵢₙ, title = "ωₐ_out")
